import heapq
from typing import List


class Solution:
    def smallestRange(self, nums: List[List[int]]) -> List[int]:
        # heap holds (value, list_index, element_index) — one entry per list,
        # always the smallest not-yet-consumed value from that list
        heap = [(nums[i][0], i, 0) for i in range(len(nums))]
        heapq.heapify(heap)

        current_max = max(lst[0] for lst in nums)
        best_start, best_end = -10**9, 10**9

        while True:
            val, i, j = heapq.heappop(heap)

            if current_max - val < best_end - best_start:
                best_start, best_end = val, current_max

            # this list is exhausted; no full window can be formed anymore
            if j + 1 == len(nums[i]):
                break

            next_val = nums[i][j + 1]
            current_max = max(current_max, next_val)
            heapq.heappush(heap, (next_val, i, j + 1))

        return [best_start, best_end]
